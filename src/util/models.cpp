...
std::string fetch(const ModelSpec& spec, const paths::fs::path& dest,
    ...
    if (spec.url.empty()) {
        return "Bilinmeyen model: " + spec.id +
               " — Ayarlar'dan dosya yolunu elle verin.";
    }
    if (!net::can_download()) {
        return "İndirme aracı (curl) bulunamadı. " + spec.label + " dosyasını " +
               "elle indirip Ayarlar'dan yolunu gösterin:\n" + spec.url;
    }

    const std::string prefix = spec.label + " indiriliyor";
...
std::string ensure_whisper_model(const Settings& s, const ProgressFn& progress) {
    ...
    if (!s.whisper_model_path.empty()) {
        ...
        return "Whisper modeli bulunamadı: " + paths::to_utf8(dest);
    }
...
std::string ensure_diarization_models(const Settings& s, const ProgressFn& progress) {
    ...
    if (!has_file(seg)) {
        if (!s.diar_segmentation_model.empty())
            return "Bölütleme modeli bulunamadı: " + paths::to_utf8(seg);
    ...
    if (!has_file(emb)) {
        if (!s.diar_embedding_model.empty())
            return "Ses izi modeli bulunamadı: " + paths::to_utf8(emb);
